...
def analyze_palm(image_data):
    try:
        ...
        # 特殊線をより派手に出すようにした system_prompt（修正版）
        system_prompt = (
            "あなたはプロの手相鑑定士です。以下の条件に従って、手相画像から5つの線・相を選び、"
            "それぞれ意味と印象をわかりやすく説明してください。\n\n"
            "【出力構成】\n"
            "・1. 生命線、2. 運命線、3. 金運線は必ず含める\n"
            "・4. 特殊線1、5. 特殊線2は以下の中からできるだけ選ぶこと：\n"
            f"{special_lines_text}\n"
            "・特殊線は少しでも見える兆しや傾向があれば積極的に採用してOKです\n"
            "・占いとして印象的になる線を優先的に紹介してください\n"
            "・2つの特殊線がどうしても見つからない場合のみ、感情線や頭脳線で自然に補完してください\n\n"
            "【各線の意味ガイド】\n"
            f"{description_text}\n\n"
