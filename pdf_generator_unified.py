...
def draw_shincom_a4(c, data, include_yearly=False):
    ...
    if data.get("birthdate"):
        try:
            y_int, m_int, d_int = map(int, data["birthdate"].split("-"))
            birthdate_str = f"{y_int}年{m_int}月{d_int}日"
        except Exception:
            birthdate_str = str(data["birthdate"])
        c.setFont(FONT_NAME, 12)
        c.drawCentredString(width/2, y + 5 * mm, f"生年月日：{birthdate_str}")
    c.setFont(FONT_NAME, 12)
    for i in range(3):
